 ...
 * 
 * \brief High level interface that the message objects (water,
 *        FA, OTA) use to transmit and receive messages to/from
 *        the modem. 
 * 
 ...
 * \li Send The Primary Command to Send (M_COMMAND_SEND_DATA, 
 ...
 * Status and Number of pending OTA messages.
 *
 */
...
static void parseModemOtaCmdResponse(modemCmdReadData_t *readDataP) {
    if (readDataP->valid && (readDataP->modemCmdId == M_COMMAND_GET_INCOMING_PARTIAL)) {
        ...
        // we are only interested in 8 bits since we will never retrieve more that 32 bytes at a time
